...
def calculate_climatology_multi_var(initial_year,final_year,ensemble_data, observations, variables=["t2m", "r", "tp"]):
    ...
    era_ranks = era_ranks.sel(time=slice("%s-01-01"%initial_year, "%s-12-31"%final_year))
        ...
        if len(matching_positions) > 5:
            ...
            for var in variables:
                ...
                if weekly_means:
                    ...
                    clim_data = clim_data.assign_coords(number=np.arange(len(weekly_means)))
...
def calculate_acc_maps(data_corrected, data_uncorrected, era_week1, era_week2, climatology_datasets,p_value=0.05):
    ...
    # Create figure
    fig, axs = plt.subplots(3, 4, sharey=True, figsize=(13, 10), 
    ...
    def setup_map(ax):
        ...
        ax.add_feature(cfeature.LAKES, edgecolor='darkblue', facecolor='darkblue', alpha=0.5)
        ax.add_feature(cfeature.RIVERS, edgecolor='darkblue', alpha=0.5)
        ax.set_xlim(104.25, 107.5)
        ax.set_ylim(8.5, 12)
        ax.plot(106.7, 10.77, 'ko', markerfacecolor="yellow", markersize=7, transform=ccrs.Geodetic())
        ax.text(106, 11, 'HCMC', transform=ccrs.Geodetic(), fontsize=fontsize-2, color="k")
    ...
    def calculate_acc(forecast_data, obs_data, climatology, var_name):
        ...
        forecast_filtered = forecast_data.where(
            (forecast_data.time.dt.month > 4) & (forecast_data.time.dt.month < 11), drop=True)
        forecast_filtered = forecast_filtered.where(
            (forecast_filtered.valid_time.dt.month > 4) & (forecast_filtered.valid_time.dt.month < 11), drop=True)
        climatology_filtered = climatology.where(
            (climatology.time.dt.month > 4) & (climatology.time.dt.month < 11), drop=True)
        
        climatology_filtered = climatology_filtered.groupby("time.dayofyear").mean(dim="time")
        ...
        obs_filtered = obs_filtered.where(obs_filtered.time.isin(dates_inter), drop=True)
        o = obs_filtered.squeeze().groupby("time.dayofyear")
        
        # Calculate anomalies
        F, A = f - climatology_filtered, o - climatology_filtered
        F, A = F.where(np.isnan(F) == False, drop=True),A.where(np.isnan(A) == False, drop=True)
        ...
        weights = np.repeat(np.repeat(weights, axis=2, repeats=F.lon.size), 
                          axis=0, repeats=axis0)
        ...
        weighted_f = var_data_f - ((weights * var_data_f).sum(dim="time")) / np.sum(weights, axis=0)
        weighted_a = var_data_a - ((weights * var_data_a).sum(dim="time")) / np.sum(weights, axis=0)
        
        numerator = np.sum(weights * np.array(weighted_f) * np.array(weighted_a), axis=0)
        denom_f = np.sum(np.array(weights * (var_data_f - var_data_f.mean(dim="time"))**2), axis=0)
        denom_a = np.sum(np.array(weights * (var_data_a - var_data_a.mean(dim="time"))**2), axis=0)
        ...
        for la in range(len(F.lat)):
            for lo in range(len(F.lon)):
                if var_name == "tp":
                    ...
                    p_vals[la, lo] = pearsonr(var_data_f[:, la, lo], var_data_a[:, la, lo]).pvalue
        
        p_vals = np.where(p_vals < p_value, 1, 0)# Here, we replace those p values that below p_value to 0, (p values below p_value shows
    ...
    for dataset_idx, (dataset_name, dataset) in enumerate(datasets):
        for var_idx, (var_name, var_config) in enumerate(variables.items()):
            for lead_idx in range(2):  # 1 and 2 week lead times
                ...
                # Get forecast data
                forecast_data = dataset[var_name].where(dataset['step.days'] == (7 + 7*lead_idx), drop=True).mean(dim="number")
                ...
                contador += 1
                cs = axs[ax_idx].contourf(dataset["lon"], dataset["lat"], acc, 
                                        transform=ccrs.PlateCarree(), levels=acc_levels, cmap='PiYG')
    ...
    for i, pos in enumerate(positions):
        axs[pos].text(103.5, 10.5, labels[i], transform=ccrs.Geodetic(), 
                     color="k", fontsize=fontsize)
    
    # Add dataset labels
    axs[0].text(105.4, 12.9, 'ACC raw forecast', transform=ccrs.Geodetic(), 
               color="k", fontsize=fontsize+1)
    axs[2].text(105.4, 12.9, 'ACC calibrated forecast', transform=ccrs.Geodetic(), 
               color="k", fontsize=fontsize+1)
    
    # Add colorbar
    cbar = fig.colorbar(cs, fig.add_axes([0.92, 0.08, 0.025, 0.8]), orientation='vertical')
...
fig = calculate_acc_maps(data_corr_s, data_uncorr_s, era_week1, era_week2, climatology_data)#plt.savefig("Figures_paper/Fig_3_Aggregation.tiff", dpi=600)
