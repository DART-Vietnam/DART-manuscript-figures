...
data_corr_s = xr.open_dataset("data_corr_s_github.nc")
data_uncorr_s = xr.open_dataset("data_uncorr_s_github.nc")
...
def plot_bias_maps(data, suffix):
    ...
    variables = {
        "T2m": {"levels": np.arange(-2, 2.5, 0.5), "unit": "°C", "label": "(a)", "var_key": "t2m"},
        "R": {"levels": np.arange(-10, 10.1, 1), "unit": "%", "label": "(b)", "var_key": "r"},
        "TP": {"levels": np.arange(-20, 21, 1), "unit": "mm", "label": "(c)", "var_key": "tp"}
    }
    ...
    temp = data.where((data['valid_time.month'] > 4) & 
                      (data['valid_time.month'] < 11), drop=True)
    ...
    def setup_map(ax):
        ...
        ax.text(106.5, 11, 'HCMC', transform=ccrs.Geodetic(), color="k", fontsize=fontsize-7)
    ...
    for var_name, config in variables.items():
        ...
        for i in range(2):
            ...
            forecast = forecast.mean(dim=['number'])# measure mean state in forecast simulations
            ...
            cs = ax.contourf(bias.lon, bias.lat, bias[config["var_key"]].to_numpy(),
                            levels=config["levels"], cmap='PiYG', extend="both")
            ...
            if i == 0:
                ax.text(103, 11, config["label"], transform=ccrs.Geodetic(), 
        ...
        # Add colorbar
        cbar = fig.colorbar(cs, fig.add_axes([0.68, 0.21, 0.02, 0.55]), orientation='vertical')
...
def concatenate_images(suffix):
    ...
    # Load images
    images = [Image.open(f"bias_{var}_S_{suffix}.tiff") 
              for var in ["T2m", "R", "TP"]]
